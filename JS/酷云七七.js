var rule = {
    ...
    detailUrl:'/api.php/provide/videoDetail?devid=453CA5D864457C7DB4D0EAA93DE96E66&ids=fyid&package=com.sevenVideo.app.android&version=&sj=',
...
let headers={
	...
	tkstr=listUrl.split('?')[1].split('&').map(function(it){
    return it.split('=')[1]
}).join('');
log(tkstr);
log(ts);
tkstr=listUrl.split('?')[0].replace('https://api.tyun77.cn','')+tkstr+ts+'XSpeUFjJ';
log('tk加密前:'+tkstr);
TK=md5(tkstr);
log('tk:'+TK);
headers={
